...
def create_figure_for_tag(df, tag_name):
    ...
    fig.line(x, df['flood_events'], color='red', line_dash=[1, 4],y_range_name='right axis')
    ...
    for i, (x, y) in enumerate(find_events(df)):
...
#    df = py_wrap.load_data_set()
    ...
    for tag in tags:
        plt.output_file('/volumes//Macintosh HD/Users/noelbell/Documents/big_data/Flooding Data/suncor_jetflood_plots/{0}.html'.format(tag), title='{0}'.format(tag))
        fig1 = create_figure_for_tag(df, tag)
